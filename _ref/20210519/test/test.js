...
describe('FLeX Contracts test', function () {
    ...
    describe('FLeX deploy', function () {
        ...
        it("Tip3 funtics deploy", async function () {
            walletCode = await contract.getCodeFromImage(ton, contract.tip3WalletPackage.imageBase64);
            root = await contract.deployTip3Root(ton, conf, "Funtics", "FUNT", 0, walletCode.codeBase64, 200000);
        ...
        it("funtics wallet2 deploy", async function () {
            ...
            funtics_wallet2 = await contract.Tip3Root_deployWallet(ton, conf, root, 0, internal_owner, 20000, 5000000000);
            ...
            assert.equal(wallet2_balance, 20000);
        });
        it("Print env variables for testing", async function () {
            ...
            console.log('export NUKA1_ADDR=%s', wallet1.addr.trim());
            console.log('export NUKA2_ADDR=%s', wallet2.addr.trim());
