    ...
    do {
      ...
      case 1: {
        ...
        if (1 != co_await cout().getAccountType(trade_pair.get())) {
          ...
          auto deployed_pair = co_await client_->debot_ext_in().deployTradingPair(
            tip3info.root_address, tons_cfg.trading_pair_deploy, deploy_value);
      ...
      case 2: {
        ...
        FLeXSellArgs sell_args {
          ...
          {FLeXSellArgsAddrs { my_tip3.get(), receive_wallet }}, tip3info.code, {tip3cfg}
          };
      ...
      case 3: { // buy major tip3 for minor tip3
        ...
        if (1 != co_await cout().getAccountType(trade_pair.get())) {
          ...
          auto deployed_pair = co_await client_->debot_ext_in().deployXchgPair(
            tip3info_major.root_address, tip3info_minor.root_address, tons_cfg.trading_pair_deploy, deploy_value);
          co_await printf("Sell pair deployed: {address}", deployed_pair);
        }
        ...
        FLeXXchgArgs xchg_args {
          bool_t{false}, price, uint128{one_full_ton_size}, amount, minor_amount, lend_finish_time, min_amount, deals_limit,
          tons_value, price_code,
          { FLeXSellArgsAddrs { my_tip3_provide.get(), my_tip3_receive.get() }}, tip3info_major.code,
          { FLeXXchgCfgs { {tip3_cfg_major}, {tip3_cfg_minor} } }
          };

        IPriceXchgPtr price_addr(co_await client_->debot_ext_in().deployPriceXchg(build(xchg_args).endc()));
      } break;
      ...
      case 5: {
        ...
        if (select == 1) {
          ...
          FLeXCancelArgs cancel_args {
            price, min_amount, deals_limit, tons_value, price_code, tip3info.code, {tip3cfg}
            };
          ...
          auto price = uint128((co_await cout().inputTONs("Price in TONs:")).get());
          auto tons_value = uint128((co_await cout().inputTONs("Processing value (extra will return):")).get());
          FLeXXchgCancelArgs cancel_args {
            bool_t{select == 3}, price, uint128{one_full_ton_size}, min_amount, deals_limit, tons_value,
            xchg_price_code, tip3info_major.code, { FLeXXchgCfgs { {tip3_cfg_major}, {tip3_cfg_minor} } }
            };
          co_await client_->debot_ext_in().cancelXchgOrder(build(cancel_args).endc());
        }
        break;
      }
      case 6:
