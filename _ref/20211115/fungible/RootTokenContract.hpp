...
__interface IRootTokenContract {
  ...
  /// Anyone may request to deploy an empty wallet (using internal message with attached crystals).
  [[internal, noaccept, answer_id]]
  address deployEmptyWallet(
    uint256     pubkey,  ///< New wallet's public key.
    address_opt owner,   ///< New wallet's internal owner address.
    uint128     crystals ///< Crystals to be sent to the deployable wallet.
                         ///< Must be less that attached (to the call) crystals value.
  ) = 13;
  ...
  void grant(
    ...
    uint128 crystals ///< Amount of crystals to be attached to message.
                     ///< If the method called by internal message, will be ignored
                     ///<  (rest attached crystals will be transmitted).
  ) = 14;
