...
struct stTONsDetails {
  ...
  bool_t transferring_stake_back; ///< If the contract is in "transferring stake back" state
...
__interface IstTONs {
  ...
  void lendOwnership(
    address answer_addr,      ///< Answer address
    uint128 crystals,         ///< Crystals to send with the message (ignored for internal-ownership requests)
   ...
   * \param processing_crystals Value will be attached to the message
   */
  [[external, internal, noaccept]]
  void returnStake(address dst, uint128 processing_crystals) = 13;

  /** Eliminate contract and return all the remaining crystals to \p dst
...
struct DstTONs {
  ...
  bool_t transferring_stake_back_;       ///< If the contract is in "transferring stake back" state
