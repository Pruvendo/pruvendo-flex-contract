...
public:
  ...
  // just performing pubkey check and accepting message
  __always_inline
  void onDeploy() {
    require(msg_pubkey() == owner_, error_code::message_sender_is_not_my_owner);
    tvm_accept();
  ...
  // =============== Support functions ==================
  DEFAULT_SUPPORT_FUNCTIONS(IProxy, proxy_replay_protection_t)
...
DEFAULT_MAIN_ENTRY_FUNCTIONS(Proxy, IProxy, DProxy, PROXY_TIMESTAMP_DELAY)
