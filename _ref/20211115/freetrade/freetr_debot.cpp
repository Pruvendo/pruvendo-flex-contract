...
public:
  ...
  resumable<void> start() override {
    ...
    if (!flex_) {
      ...
      if (initialized) {
        co_await printf("FLeX is fully initialized: {address}", flex_->get());
      } else {
        co_await printf("FLeX is NOT fully initialized: {address}", flex_->get());
        co_return;
      }
    }
    if (!client_addr_) {
    ...
    do {
      ...
      case 1: { // Buy tip3/tons
        ...
        if (1 != co_await cout().getAccountType(trade_pair.get())) {
          ...
          co_await trade_pair.deploy(state_init, Grams(deploy_value.get())).onDeploy(uint128(1), tonsCfg.trading_pair_deploy, notify_addr);
          co_await printf("Sell pair deployed: {address}", trade_pair.get());
        }

        auto min_amount = co_await trade_pair.run().getMinAmount();
      ...
      case 2: { // Sell tip3/tons
        ...
        auto ret = co_await my_tip3.tail_call<OrderRet>(price_addr, Grams(grams.get())).
          lendOwnership(*client_addr_, uint128(0), std::get<addr_std>(price_addr.val()).address, amount,
      ...
      case 3: { // buy major tip3 for minor tip3
        ...
        auto ret = co_await my_tip3_provide.tail_call<OrderRet>(price_addr, Grams(grams.get())).
          lendOwnership(*client_addr_, uint128(0), std::get<addr_std>(price_addr.val()).address, minor_amount,
                        lend_finish_time, deploy_init_cl, payload);
