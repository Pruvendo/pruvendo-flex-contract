...
: UExpression (( XMaybe (XInteger # OrderInfoLRecord) ) # ( ( XQueue OrderInfoLRecord ) # XInteger128 ) ) false  . 
...
(*  	 refine {{ IPriceCallbackPtr ( sell . client_addr ) ( Grams ( sell . account . get ( ) ) ) . onOrderFinished ( ret , bool_t { true } ) ; { _ } }} .  *)
 	 refine {{ ({sell_opt}) -> reset () }} . 
 refine {{ if ( ~ 1 (* ((!{buy}) ^^ OrderInfo.amount ) *)  ) then { { _:UEf } } }} . 
...
(*  	 refine {{ buys_ -> change_front ( sell ) ; { _ } }} .  *)
 	 refine {{ if ( (#{buy_idx}) == ( first ((!{buy_opt}) -> get_default ()) ) ) 
                           then { { _:UEf } } }} . 
refine {{ new 'ret : ( OrderRetLRecord ) @ "ret" := {} ; { _ } }} . (* TODO: delete *)
 	 	 refine {{ (* dealer.ret_ *) {ret}
 	 	 	                       :=   [ 1 (* #{ok} *) , 
...
Parameter int_value_ : XInteger (*Grams*) .
...
: UExpression ( StateInitLRecord * XInteger256 ) false .
 ...
 Definition expected_wallet_address ( wallet_pubkey : ( XInteger256 ) ) ( internal_owner : ( XInteger256 ) ) : UExpression XInteger256 false . 
 	 	 ...
 	 	 refine {{ if ( 1 (*#{internal_owner}*) ) then { { _:UExpression XInteger256 false } } ; { _ } }} .
...
Definition on_sell_fail ( ec : ( XInteger ) ) 
( wallet_in : ( XAddress (* ITONTokenWalletPtrLRecord *) ) ) 
( amount : ( XInteger128 ) ) : UExpression OrderRetLRecord false . 
...
(*  	 	 refine {{ set_int_return_value ( tons_cfg_ . order_answer . get ( ) ) ; { _ } }} .  *)
 	 	 ...
 	 	 refine {{ new 'amount : ( XInteger128 ) @ "amount" := 
...
Definition buyTip3 ( amount : ( XInteger128 ) ) ( receive_tip3_wallet : ( XAddress ) ) ( order_finish_time : ( XInteger32 ) ) : UExpression OrderRetLRecord true . 
 	 	 refine {{ new ( 'sender:XAddress , 'value_gr:XInteger ) @ ( "sender" , "value_gr" ) := int_sender_and_value_ ( ) ; { _ } }} . 
 	 	 refine {{ require_ ( ( (#{ amount }) >= _min_amount_ ) , 1 (* ec::not_enough_tokens_amount *) ) ; { _ } }} . 
 	 	 refine {{ new 'cost : ( ( XMaybe XInteger ) ) @ "cost" := 
                         calc_cost_ ( #{ amount } , _price_ ) ; { _ } }} . 
 	 	 refine {{ require_ ( ( !{cost} ) , 1 (* ec::too_big_tokens_amount *) ) ; { _ } }} . 
 	 	 refine {{ require_ ( ( (!{value_gr}) > buyTip3MinValue_ ( (!{cost}) -> get_default () ) ) , 1 (* ec::not_enough_tons_to_process *) ) ; { _ } }} . 
...
: UExpression ((XQueue OrderInfoLRecord) # XInteger128) false . 
...
: URValue ((XQueue OrderInfoLRecord) # XInteger128)
...
Definition cancelSell : UExpression PhantomType false . 
 	 	 ...
 	 	 refine {{ new 'value : ( XInteger ) @ "value" := 
                               int_value ( ) ; { _ } }} . 
 	 	 refine {{ new ( 'sells : (XQueue OrderInfoLRecord) , 'sells_amount : XInteger128 )
                 @ ( "sells" , "sells_amount" ) :=
...
Definition cancelBuy : UExpression PhantomType false . 
 	 	 refine {{ new 'canceled_amount : ( XInteger128 ) @ "canceled_amount" := 
                             _buys_amount_ ; { _ } }} . 
 	 	 refine {{ new 'client_addr : ( addr_std_fixedLRecord ) @ "client_addr" := {} 
                             (* int_sender ( ) *) ; { _ } }} . 
 	 	 refine {{ new 'value : ( XInteger ) @ "value" := 
                         int_value ( ) ; { _ } }} . 
 	 	 refine {{ new ( 'buys:(XQueue OrderInfoLRecord) , 'buys_amount:XInteger128 ) 
                       @ ( "buys" , "buys_amount" ) :=
...
Definition _fallback ( x : ( XCell )) ( y : XSlice ): UExpression XInteger false . 
 	 	 refine {{ return_ 0 }} . 
 Defined . 
...
Definition prepare_price_state_init_and_addr ( price_data : ( ContractLRecord ) ) ( price_code : ( XCell ) ) : UExpression ( StateInitLRecord * XInteger256 ) false . 
