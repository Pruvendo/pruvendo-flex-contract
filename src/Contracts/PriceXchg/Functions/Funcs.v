...
Import ClassTypesNotationsModule.
...
Local Open Scope xlist_scope.
 ...
 Definition minor_cost (amount: uint128) 
	...
	refine {{ if ( ( ?((!{cost}) >> #{128})) \\ ( (!{cost}) == 0 ) ) 
		then { { _:UExpression (optional uint128) false } } ; { _ } }} . 
		refine {{ return_ {} }} . 
...
Definition make_deal ( sell : ULValue OrderInfoXchgLRecord) ( buy : ULValue OrderInfoXchgLRecord ) 
					 : UExpression ( boolean # (boolean # uint128) ) false . 
	...
	refine {{ new 'last_tip3_buy : boolean @ "last_tip3_buy" := 	 	 
			 ( !{deal_amount} == ( (!{buy}) ↑ OrderInfoXchg.amount ) ) ; { _ } }} .
	refine {{ new 'buy_payment : optional uint128 @ "buy_payment" := 
	...
	refine {{ new 'transaction_costs : uint128 @ "transaction_costs" := 
		( (_tons_cfg_ ↑ TonsConfig.transfer_tip3) * #{2} ) + (_tons_cfg_ ↑ TonsConfig.send_notify) ; { _ } }} . 
	...
	refine {{ new 'sell_out_of_tons : ( boolean ) @ "sell_out_of_tons" := 
		(!{ sell_ton_costs } ) > ( (!{sell}) ↑ OrderInfoXchg.account ) ; { _ } }} . 
	refine {{ new 'buy_out_of_tons : boolean @ "buy_out_of_tons" := 
...
( all_amount : ULValue ( uint128 ) ) 
( sell : URValue ( boolean ) a5 ) 
: URValue ( (optional ( uint # OrderInfoXchgLRecord )) # ( (XQueue OrderInfoXchgLRecord) # uint128 ) )  
...
Definition process_queue (sell_idx : uint) 
 	 	 ...
 	 	 refine {{ new ( 'buy_idx_cur:uint , 'buy:OrderInfoXchgLRecord ) @ ( "sell_idx_cur" , "sell" ) 
 	 	 	 ...
 	 	 	 refine {{ if ( (++ ({deals_count})) > _deals_limit_ ) then { { _:UEt } } ; { _ } }} . 
...
Definition int_sender_and_value : UExpression ( raw_address # Grams ) false . 
	refine {{ new 'sender : ( raw_address ) @ "sender" :=  int_sender () ; { _ } }} . 
	refine {{ return_ [ !{ sender } , int_value () ] }} . 
Defined . 

Definition int_sender_and_value_right : URValue ( raw_address #  Grams ) false := 
...
Notation " 'set_int_sender_' '(' ')' " := ( set_int_sender ) 
...
Notation " 'set_int_return_value_' '(' ')' " := ( set_int_sender ) 
 												( in custom ULValue at level 0 ) : ursus_scope .
...
Definition verify_tip3_addr_right { a1 a2 a3 a4 } ( cfg : URValue Tip3ConfigLRecord a1 ) 
 												  ( tip3_wallet : URValue raw_address a2 ) 
...
Definition on_ord_fail_right { a1 a2 a3 }  ( ec : URValue uint a1 ) 
                                           ( wallet_in : URValue raw_address (* ITONTokenWalletPtrLRecord *) a2 ) 
...
Definition process_queue_impl ( tip3root_sell : raw_address ) 
							  ...
							  ( buys_amount : uint128 ) 
							  ( buys : queue OrderInfoXchgLRecord ) : UExpression process_retLRecord false . 
...
Definition process_queue_impl_right { a1 a2 a3 a4 a5 a6 a7 a8 a9 a10 a11 a12 }  
									...
									: URValue process_retLRecord ( orb ( orb ( orb ( orb ( orb ( orb ( orb ( orb ( orb ( orb ( orb a12 a11 ) a10 ) a9 ) a8 ) a7 ) a6 ) a5 ) a4 ) a3 ) a2 ) a1 ) := 
...
Definition onTip3LendOwnership ( answer_addr : raw_address ) 
							   ( balance : uint128 ) 
							   ( lend_finish_time : uint32 )
							   ( pubkey : uint256 )
							   ( internal_owner : raw_address ) 
							   ( payload : TvmCell ) : UExpression OrderRetLRecord true . 
 	 	 refine {{ new ( 'tip3_wallet : raw_address , 'value : uint (* Grams *) ) @ 
                 ( "tip3_wallet" , "value" ) := int_sender_and_value_ ( ) ; { _ } }} . 
  	 refine {{ new 'wallet_in : raw_address @ "wallet_in" (* ITONTokenWalletPtr *) := !{tip3_wallet} ; { _ } }} . 
