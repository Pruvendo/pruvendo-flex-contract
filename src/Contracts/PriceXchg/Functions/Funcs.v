...
Definition make_deal ( sell : ULValue ( OrderInfoXchgLRecord ) ) ( buy : ULValue ( OrderInfoXchgLRecord ) ) : UExpression ( XBool * (XBool * XInteger128) ) false . 
 	 	 ...
 	 	 refine {{ new 'sell_ton_costs : ( XInteger128 ) @ "sell_ton_costs" := 0 ; { _ } }} . 
 	 	 refine {{ new 'buy_ton_costs : ( XInteger128 ) @ "buy_ton_costs" := 0 ; { _ } }} . 
 	 	 refine {{ new 'transaction_costs : ( XInteger128 ) @ "transaction_costs" := 
...
( all_amount : ULValue ( XInteger128 ) ) 
( sell : ( XBool ) ) : 
...
: URValue ( (XMaybe ( XInteger # OrderInfoXchgLRecord )) # ( (XQueue OrderInfoXchgLRecord) # XInteger128 ) )  
...
Definition process_queue ( sell_idx : ( XInteger ) ) ( buy_idx : ( XInteger ) ) : UExpression PhantomType false . 
 	 	 refine {{ new 'sell_opt : ( XMaybe (XInteger # OrderInfoXchgLRecord) ) @ "sell_opt" := {} ; { _ } }} . 
 	 	 refine {{ new 'buy_opt  : ( XMaybe (XInteger # OrderInfoXchgLRecord) ) @ "buy_opt"  := {} ; { _ } }} . 
 	 	 ...
 	 	 refine {{ new ( 'sell_idx_cur:XInteger , 'sell:OrderInfoXchgLRecord ) @ ( "sell_idx_cur" , "sell" ) 
                                 := ( (!{sell_opt}) -> get_default () ) ; { _ } }} . 
 	 	 refine {{ new ( 'buy_idx_cur:XInteger , 'buy:OrderInfoXchgLRecord ) @ ( "sell_idx_cur" , "sell" ) 
 	 	 	 ...
 	 	 	 refine {{ if ( (++ ({deals_count})) > _deals_limit_ ) then { { _:UEf } } ; { _ } }} . 
 	 	 	 	 refine {{ new 'half_process_queue:XInteger @ "" := (_tons_cfg_ ^^ TonsConfig.process_queue) / #{2} ; { _ } }} . 
...
(*  	 refine {{ IPriceCallbackPtr ( sell . client_addr ) ( Grams ( sell . account . get ( ) ) ) . onOrderFinished ( ret , bool_t { true } ) ; { _ } }} .  *)
 	       refine {{ {sell_opt} -> reset () }} . 
       refine {{ if ( ~ {_:URValue XInteger false} ) then { { _:UEf } } }} . 
...
(*  	 refine {{ buys_ ^^ change_front ( buy ) ; { _ } }} .  *)
 ...
 Definition int_sender_and_value : UExpression ( XAddress # XInteger (* Grams *) ) false . 
...
( wallet_pubkey : ( XInteger256 ) ) 
( internal_owner : ( XInteger256 ) ) 
: UExpression XInteger256 false . 
...
Definition verify_tip3_addr ( cfg : ( Tip3ConfigLRecord ) ) ( tip3_wallet : ( XAddress ) ) ( wallet_pubkey : ( XInteger256 ) ) ( internal_owner : ( XInteger256 ) ) : UExpression XBool false . 
 refine {{ new 'expected_address : ( XInteger256 ) @ "expected_address" := 
...
: UExpression OrderRetLRecord false . 
  	 ...
  	 refine {{ new 'wallet_in:XAddress @ "wallet_in" (* ITONTokenWalletPtr *) := !{tip3_wallet} ; { _ } }} . 
 	 	 ...
 	 	 refine {{ new 'min_value : ( XInteger128 ) @ "min_value" := 
 	 	 ...
 	 	 refine {{ new 'amount : ( XInteger128 ) @ "amount" := (!{args}) ^^ PayloadArgs.amount ; { _ } }} . 
 	 	 refine {{ new 'minor_amount : ( XMaybe XInteger128 ) @ "minor_amount" := 
                             minor_cost_ ( !{ amount } , _price_ ) ; { _ } }} . 
 	 	 refine {{ new 'err : ( XInteger ) @ "err" := 0 ; { _ } }} . 
 	 	 refine {{ new 'internal_owner_std : ( XInteger ) @ "internal_owner_std" := {} 
