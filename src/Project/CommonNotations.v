...
Definition ex_right {b} (x: URValue xeRecord b): URValue typ b :=
    || {x} ^^ {ex} || : _ .
    
Definition ex_left (x: ULValue xeRecord): ULValue typ :=
    {{ {x} ^^ {ex} }} : _.
    
Notation " a '↑' 'ex' " := ( ex_right a ) (in custom URValue at level 0) : ursus_scope.
Notation " a '↑' 'ex' " := ( ex_left a ) (in custom ULValue at level 0) : ursus_scope.
